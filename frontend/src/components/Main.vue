<template>
  <div class="main">
    <div class="filters">
      ...
      <br>
      <!-- First table -->
      <el-table :data="getCategoryResults(filters.currentCategory)" height="450" style="width: 100%">
        <el-table-column :prop="filters.currentCategory" :label="filters.currentCategory" width="180" />
        <el-table-column prop="occurrence" label="occurrence" />
        <el-table-column label="ProgressBar">
          <template #default="scope">
            <el-progress :text-inside="true" :stroke-width="24" :percentage="scope.row.occurrence" status="success" />
          </template>
        </el-table-column>
      </el-table>
    </div>
    <div class="filters">
     ...
     <!-- Filters -->
        <el-select v-model="filters.currentLocation" placeholder="Select a country code">
          <el-option v-for="item in getCategoryResults('country')" :key="item.country" :label="item.country"
            :value="item.country">
          </el-option>
        </el-select>
        <el-select v-model="filters.currentGender" placeholder="Select a gender">
          <el-option v-for="item in filters.gender" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
        <el-select v-model="filters.currentAge" placeholder="Select an age">
          <el-option v-for="item in getCategoryResults('age')" :key="item.age" :label="item.age" :value="item.age">
          </el-option>
        </el-select>

        <el-select v-model="filters.currentSegment" placeholder="Select a segment">
          <el-option v-for="item in filters.segments" :key="item.value" :label="item.label" :value="item.value">
          </el-option>
        </el-select>
      </div>
       <!-- Second table -->
      ...
      <div class="counter">
        <div class="block">
      <h2 class="timer count-title count-number" >{{countTotalEvents(data.events)}}</h2>
       <p class="count-text ">Total events</p>
       </div>
       <div class="block">
        <h2 class="timer count-title count-number" >{{countTotalEvents(getData)}}</h2>
  ...
  export default {
    async setup() {
      ...
      const getCategoryResults = ((key) => {
        ...
        //array with distinc values 
        // const locations = data.events.filter((item, index, self, key) =>
        // index === self.findIndex((t) => (
        // t.country === item.country
        // )))

        //get unique values form key
        // const unique = [...new Set(locations.map(item => item.country))];
        // const locationData = {
        //   value: unique
...
// const test = () => {
//   const filterKeys = Object.keys(filters.currentFilters);
//   return data.events.filter(item => {
//     // validates all filter criteria
//     return filterKeys.every(key => {
//       // ignores non-function predicates
//       if (typeof filters.currentFilters[key] !== 'function') return true;
//       return filters.currentFilters[key](item[key]);
//     });
//   });
// };
      const countTotalEvents = (data) => data.filter(item=> item.id).length;
      const getData = computed(() => {
        if (filters.currentLocation || filters.currentGender || filters.currentAge || filters.currentSegment) {
          return data.events.filter(i => i.country === filters.currentLocation || i.gender === filters
            .currentGender || i.age === filters.currentAge || i.segments === filters.currentSegment)
        }
        return data.events;
      });
